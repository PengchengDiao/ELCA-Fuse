...
def sobel_conv(data, channel=1):
    device = data.device
    conv_op_x = nn.Conv2d(channel, channel, (3, 3), stride=(1, 1), padding=1, groups=channel, bias=False).to(device)
    conv_op_y = nn.Conv2d(channel, channel, (3, 3), stride=(1, 1), padding=1, groups=channel, bias=False).to(device)
